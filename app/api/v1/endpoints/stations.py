...
@router.get(
    ...
    dependencies=[Depends(get_admin_or_legacy_key)],
)
async def list_stations(
    q: str | None = Query(
        None,
        min_length=1,
        description="اسم المحطة بالعربي أو الإنجليزي",
    ),
    ...
    if q:
        total, items = await station_crud.search(
            db,
            query=q,
            page=page,
            page_size=page_size,
    ...
    total, items = await station_crud.get_multi(
        db,
        page=page,
        page_size=page_size,
