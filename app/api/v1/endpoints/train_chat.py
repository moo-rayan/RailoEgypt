...
GET  /api/v1/train-chat/{train_id}/pinned
    Get pinned messages (emergency/lost/found/ticket sale).
...
async def chat_websocket(
    ...
    Client sends:
      {"type": "message", "text": "...", "msg_type": "normal|emergency|lost_item|found_item|ticket_sale"}
...
async def get_pinned_messages(
    ...
    """Get pinned messages (emergency/lost/found/ticket sale) for a train."""
    if not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Invalid authorization header")
    user = await verify_supabase_token(authorization[7:])
    if user is None:
